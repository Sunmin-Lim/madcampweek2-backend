...
const sudoPassword = process.env.SUDO_SECRET;
/**
...
function buildImage(localPath, imageName) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const cmd = `echo '${sudoPassword}' | sudo -S docker build -t ${imageName} ${localPath}`;
...
function runContainer(imageName, resources = {}) {
  ...
  if (resources.port) cmd += ` -p ${resources.port}`;
  cmd += ` ${imageName}`;

  const fullCmd = `echo '${sudoPassword}' | sudo -S ${cmd}`;
  console.log(`🚀 Running container: ${fullCmd}`);
...
function getContainerStatus(containerId) {
  return new Promise((resolve, reject) => {
    const cmd = `echo '${sudoPassword}' | sudo -S docker inspect --format='{{.State.Status}}' ${containerId}`;
...
function stopContainer(containerId) {
  return new Promise((resolve, reject) => {
    ...
    const cmd = `echo '${sudoPassword}' | sudo -S docker stop ${containerId}`;
    exec(cmd, (error, stdout, stderr) => {
      if (error) {
...
function removeContainer(containerId) {
  return new Promise((resolve, reject) => {
    ...
    const cmd = `echo '${sudoPassword}' | sudo -S docker rm -f ${containerId}`;
    exec(cmd, (error, stdout, stderr) => {
      if (error) {
